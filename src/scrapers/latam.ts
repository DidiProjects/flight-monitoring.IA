  ...
  try {
    for (const date of dateRange(startDate, endDate)) {
      ...
      if (!hasCards) {
        ...
        if (!genuineEmpty) {
          const msg = `LATAM: zero cards and no empty-state marker for ${origin}→${destination} on ${date} — likely bot/IP block.`;
...
async function isGenuineEmptyState(page: Page): Promise<boolean> {
  return await page.evaluate(() =>
    /não encontramos voos|não há voos disponíveis|nenhum resultado|não encontramos opções/i.test(document.body.innerText),
  ).catch(() => false);
}
